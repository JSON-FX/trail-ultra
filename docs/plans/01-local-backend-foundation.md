...
**Tech Stack:** Supabase CLI, Postgres 15, Deno (Edge Functions), TypeScript, `@trail-ultra/shared` (Zod), Vitest + `@supabase/supabase-js` (tests), pnpm workspaces.
...
- **Validators live once** in `@trail-ultra/shared` and are imported by both the Edge Function (Deno, via import map) and later the app.
...
- `supabase/functions/deno.json` — import map (`@trail-ultra/shared`, `zod`).
...
### Task 7: Extend `@trail-ultra/shared` validators
...
```json
{
  "imports": {
    ...
    "@trail-ultra/shared": "../../packages/shared/src/index.ts"
  }
}
```
...
import { customDataSchema, formFieldSchema, registrationInputSchema } from "@trail-ultra/shared";
